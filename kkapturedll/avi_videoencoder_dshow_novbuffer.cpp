...
public:
  ...
  // synchronous. this can be done WAY better.
  void PushData(const unsigned char *buffer)
  {
    ...
    {
      VideoCaptureTimer timer("submit");
...
public:
  ...
  // synchronous. refer to VideoSourcePin.
  void PushData(const unsigned char *buffer)
  {
    pin->PushData(buffer);
  }
...
{
  if(d->audio)
    d->audio->PushData(buffer,samples*4);
}
